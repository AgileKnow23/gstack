...
| `/icm-repo-cartographer` | **Repo Cartographer** — map a repository for agents: an `AGENTS.md` router under 60 lines, a `CLAUDE.md` pointer, one `agent-work/repo-map.yml` holding every project fact, and a Mermaid context map generated from it. Inspects and classifies before writing anything, routes work by risk and scope rather than ceremony, and validates the result with a cold-agent walk test. The YAML is the source of truth; the map never is. No scanner, no hook, no CI gate. |
