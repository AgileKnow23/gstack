...
function requireStringList(value, field, hint) {
  if (value === undefined || value === null)
    return [];
  if (!Array.isArray(value))
    fail(field, `expected a list, found ${describe(value)}. ${hint}`);
  return value.map((entry, i) => requireString(entry, `${field}[${i}]`, hint));
}
...
function parseRepoMap(text) {
  ...
  try {
    ...
    fail("(whole file)", `not valid YAML — ${err.message}. Fix the syntax, then re-run the map generator.`);
  }
  ...
  if ("summary" in projectRaw) {
    fail("project.summary", `remove it. "What is this repo and what ships out of it" belongs in ${entry} and only there — ` + `a sentence stored twice is the first fact to drift, and it drifts silently.`);
  }
  if (!Array.isArray(doc.sources_of_truth) || doc.sources_of_truth.length === 0) {
    fail("sources_of_truth", `expected a non-empty list. A repository with no source-of-truth document is one an agent has to guess at.`);
  }
  const sources = doc.sources_of_truth.map((entry2, i) => {
  ...
  const markerKeys = Object.keys(doc.risk_markers).sort();
  const expectedKeys = [...RISK_MARKER_KEYS].sort();
  if (markerKeys.join(",") !== expectedKeys.join(",")) {
    fail("risk_markers", `keys must be exactly ${expectedKeys.join(", ")} — found ${markerKeys.join(", ") || "(none)"}. ` + `The set is fixed on purpose; an empty list is the right way to say a surface does not exist here.`);
  }
  const riskMarkers = {};
  for (const key of RISK_MARKER_KEYS) {
    riskMarkers[key] = requireStringList(doc.risk_markers[key], `risk_markers.${key}`, "A list of path fragments or literals, or [] when this repo has no such surface.");
  }
  if (!Array.isArray(doc.gates) || doc.gates.length === 0) {
    fail("gates", `expected a non-empty list containing at least the "deploy" gate.`);
  }
  const gates = doc.gates.map((entry2, i) => {
    const at = `gates[${i}]`;
    if (!isPlainObject(entry2))
      fail(at, `expected a mapping, found ${describe(entry2)}.`);
    const e = entry2;
  ...
  const firstGateIndexById = new Map;
  gates.forEach((gate, i) => {
    const seen = firstGateIndexById.get(gate.id);
    if (seen !== undefined) {
      fail(`gates[${i}].id`, `duplicate gate id "${gate.id}", already defined at gates[${seen}]. Gate ids must be unique — ` + `task classes and protected boundaries resolve a gate by id, and two entries sharing one id ` + `make the answer ambiguous. Rename one, or merge them.`);
    }
  ...
  if (deploy.authority !== "user") {
    fail("gates[deploy].authority", `must be "user", found ${JSON.stringify(deploy.authority)}. Merging and deploying need explicit user authorization, every time.`);
  }
  ...
  const contexts = doc.contexts.map((entry2, i) => {
    const at = `contexts[${i}]`;
    if (!isPlainObject(entry2))
      fail(at, `expected a mapping, found ${describe(entry2)}.`);
    const e = entry2;
  ...
  const firstContextIndexByName = new Map;
  contexts.forEach((ctx, i) => {
    const seen = firstContextIndexByName.get(ctx.name);
    if (seen !== undefined) {
      fail(`contexts[${i}].name`, `duplicate context name "${ctx.name}", already defined at contexts[${seen}]. ` + `Names must be unique — depends_on resolves a context by name.`);
    }
  ...
  for (const ctx of contexts) {
    for (const id of ctx.sources_of_truth) {
      if (!seenSourceIds.has(id)) {
        fail(`contexts[${ctx.name}].sources_of_truth`, `"${id}" does not match any sources_of_truth id. Known ids: ${[...seenSourceIds].join(", ")}.`);
      }
    }
    for (const dep of ctx.depends_on) {
      if (!contextNames.has(dep)) {
        fail(`contexts[${ctx.name}].depends_on`, `"${dep}" is not a known context name. Known contexts: ${[...contextNames].join(", ")}.`);
      }
      if (dep === ctx.name) {
    ...
    for (const marker of ctx.risk) {
      if (!RISK_MARKER_KEYS.includes(marker)) {
        fail(`contexts[${ctx.name}].risk`, `"${marker}" is not a risk marker. Use one of: ${RISK_MARKER_KEYS.join(", ")}.`);
      }
    }
  }
  ...
  const protectedBoundaries = (protectedRaw ?? []).map((entry2, i) => {
    ...
    if (paths.length === 0)
      fail(`${at}.paths`, `expected at least one path, or drop the entry.`);
    return {
      paths,
  ...
  for (const [key, value] of Object.entries(doc.commands)) {
    ...
    commands[key] = value.trim() === "" ? null : value.trim();
  }
  ...
  const taskClasses = doc.task_classes.map((entry2, i) => {
  ...
  for (const tc of taskClasses) {
    if (seenClassIds.has(tc.id))
      fail("task_classes", `duplicate id "${tc.id}". Ids must be unique.`);
  ...
  return {
    ...
    project: {
      ...
      form,
      entry,
      stack: isPlainObject(projectRaw.stack) ? projectRaw.stack : {}
    },
...
class NodeIds {
  ...
  get(prefix, value) {
    ...
    if (!id) {
      throw new RepoMapError(`${prefix}[${value}]`, `internal: no Mermaid node was allocated for this reference. This is a renderer bug, not a config error.`);
    }
    return id;
  }
}
function allocateNodeIds(map) {
  const ids = new NodeIds;
  for (const ctx of map.contexts)
    ids.allocate("ctx", ctx.name);
  for (const doc of map.sources_of_truth)
    ids.allocate("sot", doc.id);
  for (const gate of map.gates)
    ids.allocate("gate", gate.id);
  map.boundaries.protected.forEach((_, i) => ids.allocate("prot", String(i)));
  for (const tc of map.task_classes)
    ids.allocate("tc", tc.id);
  return ids;
}
function label(text) {
  return text.replace(/["\r\n]/g, " ").replace(/\s+/g, " ").trim();
...
function renderMermaid(map) {
  ...
  const nodeId = (prefix, value) => ids.get(prefix, value);
  ...
  for (const gate of map.gates) {
    out.push(`    ${nodeId("gate", gate.id)}>"<b>${label(gate.id)}</b><br/>${label(gate.requires)}<br/><i>authority: ${label(gate.authority)}</i>"]`);
  }
  out.push("  end");
  ...
  for (const tc of map.task_classes) {
    if (tc.gate)
      out.push(`  ${nodeId("tc", tc.id)} ==> ${nodeId("gate", tc.gate)}`);
  }
  ...
  if (gateNodes.length > 0)
    out.push(`  class ${gateNodes.join(",")} gate;`);
  if (map.boundaries.protected.length > 0) {
...
var commandRow = (key, value) => `| ${codeCell(key)} | ${value ? codeCell(value) : "_not set_"} |`;
function renderMarkdown(map, mermaid) {
  const out = [];
  out.push(`# ${map.project.name} — agent context map`);
  out.push("");
  out.push(`> **${GENERATED_BANNER}**`);
  out.push(">");
  out.push("> The canonical source is `agent-work/repo-map.yml`. This page is rendered from it. " + "If the two disagree, the YAML wins and this page is stale — regenerate it rather than editing it.");
  out.push("");
  ...
  for (const ctx of map.contexts) {
    const risks = ctx.risk.map((r) => RISK_LABEL[r] ?? r).join(", ") || "—";
    out.push(`| **${tableCell(ctx.name)}** | ${tableCell(ctx.purpose)} | ${ctx.paths.map((p) => codeCell(p)).join(", ")} | ${tableCell(risks)} |`);
  }
  out.push("");
  ...
  out.push("");
  for (const gate of map.gates) {
    out.push(`- **${tableCell(gate.id)}** — ${tableCell(gate.when)}. Requires: ${tableCell(gate.requires)} Authority: **${tableCell(gate.authority)}**. An agent never clears it.`);
  }
  out.push("");
  out.push("## Commands");
  out.push("");
