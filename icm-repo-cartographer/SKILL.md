...
the repo already has an `AGENTS.md` you were about to overwrite.
...
## Phase 1 — Inspect before you touch
...
**Ask for approval. Do not write until you have it.** This is a method built on
human gates; the first one is yours to honor.

If the repo already has an `AGENTS.md` with real content, ask explicitly whether to
absorb it or replace it. Never silently overwrite an entry file.
...
The only file that carries project facts. Field-by-field guidance is in
...
- `generated_map` carries only `direction` and `source_of_truth: false`. Where the
  map is written and what it is called is fixed tool policy —
  `agent-work/generated/agent-map.{mmd,md}` — and naming an `output_dir`,
  `mermaid` or `markdown` field fails the parse. A config that could pick a
  destination is a file writer, not a repository map.
- **Unknown fields fail the parse.** A key this schema does not know is never
...
It writes exactly two files, always named `agent-map.mmd` and `agent-map.md`, into
`agent-work/generated/`. It checks every existing component of those paths before
the first byte is written and refuses a symlink anywhere along the way — the
failure it is preventing is overwriting a file nobody asked it to touch.

`--out <dir>` renders into a different directory. That is an operator override for
a one-off; the config has no say in the destination, and the filenames do not
change.
